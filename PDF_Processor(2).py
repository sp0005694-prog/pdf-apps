...
st.set_page_config(page_title="PDF Image Processor 1.2", layout="wide")
...
def create_pdf_from_images(images):
    ...
    try:
        ...
        first_img.save(img_buffer, format='PNG')
        img_buffer.seek(0)
...
def create_word_document_with_options(images, orientation="portrait", margins="normal"):
    """Create Word document with precise image sizing and clean layout - NO GAPS"""
    if not DOCX_AVAILABLE:
...
if uploaded_pdf:
        ...
        if REPORTLAB_AVAILABLE:
            with cols[col_index]:
                ...
                try:
                    ...
                    st.download_button(
                        label="📄 Download as PDF (Exact Size)",
                        data=pdf_bytes,
        ...
        if DOCX_AVAILABLE:
            with cols[col_index]:
                ...
                try:
                    ...
                    st.caption("No gaps between images in Word document")
                except Exception as e:
    ...
    - **Preserves cropping**: Maintains all your cropping work
    - **All pages included**: Complete PDF with all processed pages
    
    ### 📝 **Word Export**
    - **2-column layout**: Clean, organized document layout
    - **No gaps**: Images placed directly next to each other
    - **Professional formatting**: Ready-to-use document
    """)
