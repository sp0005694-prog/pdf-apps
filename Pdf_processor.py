...
try:
    ...
    st.error("PyMuPDF (fitz) not installed. Please add 'PyMuPDF>=1.23.0' to requirements.txt")
    st.stop()

try:
    from PIL import Image, ImageDraw
except ImportError:
    st.error("Pillow not installed. Please add 'Pillow>=10.0.0' to requirements.txt")
    st.stop()

try:
    import numpy as np
except ImportError:
    st.error("NumPy not installed. Please add 'numpy>=1.24.0' to requirements.txt")
    st.stop()

try:
    import img2pdf
except ImportError:
    st.error("img2pdf not installed. Please add 'img2pdf>=0.4.0' to requirements.txt")
    st.stop()
...
try:
    ...
    st.error("Standard library modules missing. This should not happen.")
    st.stop()
...
st.set_page_config(
    page_title="PDF Image Processor 1.2",
    page_icon="🔄",
    layout="wide",
    initial_sidebar_state="expanded"
)

st.title("🔄 PDF Image Processor 1.2")
st.markdown("**Visual Logo Selection + Freeform Polygon + Both-Direction Cropping**")
...
# ... (rest of your functions remain the same, just add try-except blocks to each)
...
# ... (rest of your functions with try-except blocks)
...
# Main App Flow
st.sidebar.header("⚙️ PDF Image Processor 1.2")
...
st.markdown("---")
st.caption("PDF Image Processor 1.2 | Visual Logo Selection + Freeform Polygon + Both-Direction Cropping")
